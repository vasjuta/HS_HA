...
Endpoints: /hello (for handshake)
           /predict_genres
connects to GenresClassifier - single entry point
"""
...
app = Sanic("movies_classifier")

app.ctx.classifier = GenresClassifier()


@app.post("/predict_genres")
def classify_handler(request: Request) -> HTTPResponse:
    input_param = request.json
    return json({"result": app.ctx.classifier.predict_genres(input_param["plot"])})
